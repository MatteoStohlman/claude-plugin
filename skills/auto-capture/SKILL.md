---
name: auto-capture
description: Automatically capture ACT NOW items and a session summary to Vello when a work session is ending. Use when wrapping a brainstorm, parking a project, finishing a Panning for Gold run, or otherwise closing a session with decisions worth remembering. Use the Vello capture tool available in the current client (often named `capture_thought`; prefixes vary by connector). This is a behavioral protocol, not a background hook.
...
3. Before capturing an ACT NOW item, check for an obvious existing match in Vello using the available search tool if the client exposes one (often `search_thoughts`; prefixes vary by connector).
4. Capture each ACT NOW item as its own self-contained thought using the available Vello capture tool.
...
- one Vello capture per ACT NOW item
- one Vello capture for the session summary
...
- If the capture tool fails, do not invent success. Tell the user the local wrap-up succeeded but the Vello capture did not.
- Tool names vary by client and connector. Use the Vello search/capture tools available in the current environment rather than assuming a fixed prefix.
- This skill is intentionally reusable. The auto-capture half of the flywheel pairs naturally with Panning for Gold (upstream evaluation) and any live-retrieval workflow at session start (downstream surfacing).
- Originally adapted from the OB1 Auto-Capture skill by Jared Irish.
